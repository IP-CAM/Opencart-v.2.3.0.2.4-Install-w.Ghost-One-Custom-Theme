...
class Cart {
	...
	public function getCartData($session_id, $product_id)
    {
        ...
        $cart_id_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE session_id = '" . $this->db->escape($session_id) . "'");

        foreach ($cart_id_query->rows as $cart) {
            $total_count += $cart['quantity'];
            if ($cart['product_id'] == $product_id) {
                $cart_data['quantity'] = $cart['quantity'];
            }

            $cart_data['cart_id'] = $cart['cart_id'];
            $cart_data['customer_id'] = $cart['customer_id'];
            $cart_data['count'] = $total_count;
        }

        return $cart_data;
    }

    public function getRemoveCartData($session_id, $product_id)
    {
        $cart_data = array();
